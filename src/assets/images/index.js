// general
export { default as sfondoLegno } from "./sfondo-legno.webp";
export { default as logo } from "./logo.webp";

// icons
export { default as checkIcon } from "./check.svg";
export { default as arrow } from "./arrow.svg";
export { default as arrowWood } from "./arrow-wood.svg";
export { default as homeIcon } from "./navbar/home-icon.svg";
export { default as servicesIcon } from "./navbar/services-icon.svg";
export { default as blogIcon } from "./navbar/blog-icon.svg";
export { default as contactsIcon } from "./navbar/contacts-icon.svg";
export { default as logoutIcon } from "./navbar/logout-icon.svg";
export { default as hamburgerIcon } from "./navbar/hamburger-icon.svg";
export { default as closeIcon } from "./navbar/close-icon.svg";
export { default as emailIcon } from "./login/email-icon.svg";
export { default as lockIcon } from "./login/lock-icon.svg";
export { default as warningIcon } from "./login/warning-icon.svg";
export { default as writeArticle } from "./personal-area/write-article.svg";
export { default as trashIcon } from "./personal-area/trash-icon.svg";
export { default as modifyIcon } from "./personal-area/modify-icon.svg";
export { default as addImgIcon } from "./personal-area/add-img-icon.svg";
export { default as phoneIcon } from "./contacts/phone-icon.svg";
export { default as instagramIcon } from "./contacts/instagram-icon.svg";
export { default as whatsappIcon } from "./contacts/whatsapp-icon.svg";
export { default as facebookIcon } from "./contacts/facebook-icon.svg";
export { default as contactsEmailIcon } from "./contacts/contacts-email-icon.svg";

// images cosa offro
export { default as manichino } from "./cosa-offro/manichino-cosa-offro.webp";

// about me
export { default as luci } from "./about-me/luci.webp";
export { default as firstSlideRight } from "./about-me/PrimaSlideDX.webp";
export { default as firstSlideLeft } from "./about-me/PrimaSlideSX.webp";
export { default as secondSlideRight } from "./about-me/SecondaSlideDX.webp";
export { default as SecondSlideLeft } from "./about-me/SecondaSlideSX.webp";
export { default as thirdSlideRight } from "./about-me/TerzaSlideDX.webp";
export { default as thirdSlideLeft } from "./about-me/TerzaSlideSX.webp";

// esperienze
export { default as experiencePath } from "./esperienze/experience-path.svg";
export { default as experiencePathMobile } from "./esperienze/experience-path-mobile.svg";
export { default as manichinoExp } from "./esperienze/manichino-esperienze.webp";

// serivizi
export { default as serviziCartellino } from "./servizi/servizi-cartellino.webp";
export { default as cartellinoMobile } from "./servizi/cartellino-mobile.webp";
export { default as serviziSfondo } from "./servizi/servizi-sfondo.webp";
export { default as allestimentoPuntoVendita } from "./servizi/allestimentoPuntoVendita.webp";
export { default as layout } from "./servizi/layoutCartellino.webp";
// allestimento vetrine
export { default as allestimentoVetrine1 } from "./servizi/allestimento-vetrine-sc/allestimentoVetrine2.webp";
export { default as allestimentoVetrine2 } from "./servizi/allestimento-vetrine-sc/2.webp";
export { default as allestimentoVetrine3 } from "./servizi/allestimento-vetrine-sc/3.webp";
// allestimento punto vendita
export { default as allestimentoPuntoVendita1 } from "./servizi/allestimento-punto-vendita-sc/1.webp";
export { default as allestimentoPuntoVendita2 } from "./servizi/allestimento-punto-vendita-sc/2.webp";
export { default as allestimentoPuntoVendita3 } from "./servizi/allestimento-punto-vendita-sc/3.webp";
// layout negozio
export { default as layoutNegozio1 } from "./servizi/layout-negozio-sc/1.webp";
export { default as layoutNegozio2 } from "./servizi/layout-negozio-sc/2.webp";
export { default as layoutNegozio3 } from "./servizi/layout-negozio-sc/layout.webp";
// layout vetrine
export { default as layoutVetrine1 } from "./servizi/layout-vetrine-sc/1.webp";
export { default as layoutVetrine2 } from "./servizi/layout-vetrine-sc/2.webp";
export { default as layoutVetrine3 } from "./servizi/layout-vetrine-sc/3.webp";
// formazione
export {default as formazione } from "./servizi/comingSoon.webp";

// blog
export { default as searchIcon } from "./blog/search-icon.svg";
export { default as manichinoBlog } from "./blog/manichino-blog.webp";
export { default as eyeIcon } from "./blog/eye-icon.svg";

// login
export { default as manichinoLogin } from "./login/manichino-login.webp";

// placeholder
export { default as placeholder } from "./placeholder.webp";
